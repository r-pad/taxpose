...
class PointCloudTrainingModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for key, val in log_values.items():
            self.log(f"train/{key}", val, batch_size=batch_size)
        ...
        if (self.global_val_step % self.image_log_period) == 0:
            ...
            for key, val in results_images.items():
                if (isinstance(val, wandb.Object3D), isinstance(val, wandb.Html)):
                    wandb.log(
                        {
                            f"val_{dataloader_idx}_plots/{key}": val,
                            "trainer/global_step": self.global_step,
                        }
                    )
                else:
                    self.logger.log_image(
                        f"val_{dataloader_idx}_plots/{key}",
...
class AdversarialPointCloudTrainingModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        (
            pzX_loss,
            pzX_log_values,
            pred_points_action_orig_frame,
        ) = self.adversarial_module_step(batch, batch_idx)

        gen_opt.zero_grad()
        disc_opt.zero_grad()
        self.manual_backward(pzX_loss, retain_graph=True)
        self.clip_gradients(
            gen_opt, self.gradient_clipping, gradient_clip_algorithm="norm"
        )
        gen_opt.step()
        ...
        real_logits, _ = self.discriminator(
            points_action.permute(0, 2, 1), points_anchor.permute(0, 2, 1)
        )
        fake_logits, _ = self.discriminator(
            pred_points_action_orig_frame.permute(0, 2, 1).detach(),
            points_anchor.permute(0, 2, 1),
        )
        ...
        real_pred = F.binary_cross_entropy_with_logits(
            real_logits,
            torch.full_like(real_logits, self.real_label_val),
            reduction="mean",
        )
        fake_pred = F.binary_cross_entropy_with_logits(
            fake_logits,
            torch.full_like(fake_logits, self.fake_label_val),
            reduction="mean",
        )
        ...
        self.clip_gradients(
            disc_opt, self.gradient_clipping, gradient_clip_algorithm="norm"
        )
        disc_opt.step()
        ...
        fake_logits, _ = self.discriminator(
            pred_points_action_orig_frame.permute(0, 2, 1),
            points_anchor.permute(0, 2, 1),
        )
        ...
        generator_loss = (
            ...
            * F.binary_cross_entropy_with_logits(
                fake_logits,
                torch.full_like(fake_logits, self.real_label_val),
                reduction="mean",
            )
        )
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        for key, val in log_values.items():
            self.log(
                f"val_{key}/val_{dataloader_idx}",
                val,
                add_dataloader_idx=False,
                batch_size=batch_size,
            )
        ...
        if (self.global_val_step % self.image_log_period) == 0:
            ...
            for key, val in results_images.items():
                if (isinstance(val, wandb.Object3D), isinstance(val, wandb.Html)):
                    wandb.log(
                        {
                            f"val_{key}/val_{dataloader_idx}": val,
        ...
        self.log(
            f"val_loss/val_{dataloader_idx}",
            loss,
            add_dataloader_idx=False,
            batch_size=batch_size,
        )
