...
from taxpose.nets.taxposed.multilaterate import MultilaterationHead

# Share a model with the other script
from taxpose.nets.taxposed.multimodal_transformer_flow import DGCNN
from taxpose.nets.taxposed.pointnet2 import PointNet2MSG, PointNet2SSG
from taxpose.nets.taxposed.pointnet2pyg import PN2DenseParams, PN2DenseWrapper
from taxpose.nets.taxposed.relative_encoding import (
...
from taxpose.utils.sample_utils import sample_uniform_cube, sample_uniform_random
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask, src_pe=None, tgt_pe=None):
        ...
        return self.decode(
            self.encode(src, src_mask, src_pe, tgt_pe),
            src_mask,
            tgt,
            tgt_mask,
            src_pe,
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask, src_pe, tgt_pe):
        return self.generator(
            self.decoder(
                self.tgt_embed(tgt), memory, src_mask, tgt_mask, src_pe, tgt_pe
            )
        )
...
class DecoderLayer(nn.Module):
    ...
    def __init__(
        self, size, self_attn, src_attn, feed_forward, dropout, relative=False
    ):
...
class Transformer(nn.Module):
    def __init__(
        ...
        if self.relative:
            self_attn = MultiheadRelativeAttentionWrapper(
                self.emb_dims, self.n_heads, dropout=self.dropout
            )
        else:
        ...
        self.model = EncoderDecoder(
            Encoder(
                EncoderLayer(
                    self.emb_dims, c(self_attn), c(ff), self.dropout, self.relative
                ),
                self.N,
            ),
            Decoder(
    ...
    def forward(self, *input):
        ...
        if self.relative:
            ...
            tgt_pe = input[3]
        src_embedding = (
            self.model(tgt, src, None, None, src_pe, tgt_pe)
            .transpose(2, 1)
            .contiguous()
        )
        ...
        if self.bidirectional:
            tgt_embedding = (
                self.model(src, tgt, None, None, src_pe, tgt_pe)
                .transpose(2, 1)
                .contiguous()
            )
...
class ResidualMLPHead(nn.Module):
    ...
    def __init__(
        ...
        pred_weight=True,
        residual_on=True,
        conditioning_type="old",
        conditioning_size=0,
        use_weighted_sum="all",
        use_selected_point_feature=False,
    ):
        ...
        if self.use_weighted_sum != "all":
            self.weighted_sum_k = int(
                re.match(r"^top_([0-9]*)$", self.use_weighted_sum).group(1)
            )
        ...
        elif self.conditioning_type in [
            "flow_fix-one_flow_head",
            "flow_fix-post_encoder_one_flow_head",
        ]:
        ...
        if self.flow_emb_dims < 10:
            ...
            self.proj_flow = nn.Sequential(
                PointNet(
                    [
                        self.flow_emb_dims * self.emb_mul,
                        self.flow_emb_dims * self.emb_mul // 2,
                        self.flow_emb_dims * self.emb_mul // 4,
                        self.flow_emb_dims * self.emb_mul // 8,
                    ]
                ),
                nn.Conv1d(
                    self.flow_emb_dims * self.emb_mul // 8,
    ...
    def forward(
        ...
        *input,
        ...
        if self.use_weighted_sum == "all":
            ...
            corr_points_khot, corr_points_onehots = gumbel_softmax_topk(
                scores.transpose(2, 1), k=self.weighted_sum_k, tau=1, hard=True, dim=1
            )

            selected_khot_scores = scores.transpose(2, 1) * corr_points_khot
            selected_normalized_scores = (
                selected_khot_scores / selected_khot_scores.sum(dim=1, keepdim=True)
            )

            corr_points = torch.matmul(
                anchor_points, selected_normalized_scores.contiguous()
            )
        ...
        if self.use_selected_point_feature and self.use_weighted_sum != "all":
            selected_anchor_embeddings = [
                torch.matmul(anchor_embedding, corr_points_onehot)
                for corr_points_onehot in corr_points_onehots
            ]
            selected_anchor_embeddings = torch.stack(selected_anchor_embeddings, dim=1)
            mean_selected_anchor_embeddings = torch.mean(
                selected_anchor_embeddings, dim=1
            )

            embedding = torch.cat(
                [action_embedding, mean_selected_anchor_embeddings], dim=1
            )
        ...
        if self.pred_weight:
            if self.conditioning_type in ["old-flow_weight", "flow_fix-flow_weight"]:
                weight = self.proj_flow_weight(
                    torch.cat([embedding, conditioning], dim=1)
                )
            elif self.conditioning_type in [
                "flow_fix-one_flow_head",
                "flow_fix-post_encoder_one_flow_head",
...
class ResidualFlow_DiffEmbTransformer(nn.Module):
    def __init__(
        ...
        sample=False,
        mlat_nkps=100,
        pred_mlat_weight=False,
        conditioning_type="old",
        n_heads=4,
        flow_head_use_weighted_sum="all",
        flow_head_use_selected_point_feature=False,
        post_encoder_input_dims=3,
        flow_direction="both",
        ghost_points="none",
        num_ghost_points=256,
        ghost_point_radius=0.1,
        ...
        self.conditioning_type = (
            conditioning_type  # 'old', 'flow_fix', 'old-post_encoder', 'encoder-weight'
        )
        ...
        # Only add conditioning after the encoder embedding
            ...
            head_output_dims = (
                3
                if self.conditioning_type == "flow_fix-post_encoder"
                else self.input_dims
            )
        # Add conditioning at every pointwise operation, or every pointwise operation after the encoder embedding
        elif self.conditioning_type in [
            "old-all_pointwise_naive",
            "old-post_encoder_all_pointwise",
            "flow_fix-all_pointwise_naive",
            "flow_fix-post_encoder_all_pointwise",
        ]:
            encoder_input_dims = (
                self.post_encoder_input_dims
            ...
            transformer_emb_dims = self.emb_dims + (
                self.n_heads * (self.input_dims - 3)
            )
        ...
        # Add conditioning at the encoder embedding and for the flow weight head prediction
            ...
            head_output_dims = (
                3
                if self.conditioning_type == "flow_fix-flow_weight"
                else self.input_dims
            )
        ...
        # Predict flow and weight in one head
            ...
            encoder_input_dims = (
                self.post_encoder_input_dims
                if self.conditioning_type == "flow_fix-post_encoder_one_flow_head"
                else self.input_dims
            )
            head_output_dims = 4
        else:
        ...
        if emb_nn == "dgcnn":
            ...
            assert self.conditioning_type in [
                "old-post_encoder",
                "old-post_encoder_all_pointwise",
                "flow_fix-post_encoder",
                "flow_fix-post_encoder_all_pointwise",
            ...
            self.emb_nn_action = PointNet2SSG(
                num_classes=self.emb_dims, additional_channel=encoder_input_dims - 3
            )
            self.emb_nn_anchor = PointNet2SSG(
                num_classes=self.emb_dims, additional_channel=encoder_input_dims - 3
            )
        elif emb_nn == "pn++_msg":
            print(f"--- TAXPose Decoder using 2 PN++ MSG ---")
            self.emb_nn_action = PointNet2MSG(
                num_classes=self.emb_dims, additional_channel=encoder_input_dims - 3
            )
            self.emb_nn_anchor = PointNet2MSG(
                num_classes=self.emb_dims, additional_channel=encoder_input_dims - 3
            )
        elif emb_nn == "pn++":
            print(f"--- TAXPose Decoder using 2 PN++ ---")
            args = PN2DenseParams()
            self.emb_nn_action = PN2DenseWrapper(
                in_channels=encoder_input_dims - 3, out_channels=self.emb_dims, p=args
            )
            self.emb_nn_anchor = PN2DenseWrapper(
                in_channels=encoder_input_dims - 3, out_channels=self.emb_dims, p=args
            )
        ...
        if self.relative_3d_encoding:
            ...
            self.relative_3d_pe = RotaryPositionEncoding3D(
                feature_dim=transformer_emb_dims
            )
        ...
        if self.conditioning_type.split("-")[-1] in [
            "post_encoder",
            "post_encoder_all_pointwise",
            "post_encoder_one_flow_head",
        ]:
            emb_dims_cond = (
                self.emb_dims + self.input_dims - self.post_encoder_input_dims
            )
        ...
        if self.ghost_points != "none":
            ...
            self.ghost_points_encoder = nn.Sequential(
                PointNet(
                    [
                        3,
                        transformer_emb_dims // 6,
                        transformer_emb_dims // 3,
                        transformer_emb_dims,
                    ]
                ),
            )
    ...
    def forward(
        ...
        *input,
        ...
        if self.conditioning_type.split("-")[-1] in [
            ...
            action_points_dmean = action_points_dmean[
                :, : self.post_encoder_input_dims, :
            ]
            ...
            action_embedding_stack = torch.cat(
                [action_embedding, action_points[:, self.post_encoder_input_dims :, :]],
                dim=1,
            )
            anchor_embedding_stack = torch.cat(
                [anchor_embedding, anchor_points[:, self.post_encoder_input_dims :, :]],
                dim=1,
            )
        ...
        if self.conditioning_type.split("-")[-1] in [
            ...
            action_embedding = torch.cat(
                [
                    action_embedding,
                    torch.tile(action_points[:, 3:, :], (1, self.n_heads, 1)),
                ],
                dim=1,
            )
            anchor_embedding = torch.cat(
                [
                    anchor_embedding,
                    torch.tile(anchor_points[:, 3:, :], (1, self.n_heads, 1)),
                ],
                dim=1,
            )

        if self.ghost_points == "p_center":
            ghost_points_centered = sample_uniform_cube(
                self.ghost_points_radius, int(self.num_ghost_points ** (1 / 3))
            )
            ...
            anchor_embedding = torch.cat(
                [anchor_embedding, ghost_points_embedding], dim=-1
            )
        ...
        action_embedding_tf, action_attn = self.transformer_action(
            action_embedding, anchor_embedding, action_pe, anchor_pe
        )
        anchor_embedding_tf, anchor_attn = self.transformer_anchor(
            anchor_embedding, action_embedding, anchor_pe, action_pe
        )
        ...
        if self.conditioning_type.split("-")[-1] in [
            ...
            action_embedding_tf = torch.cat(
                [action_embedding_tf, action_points[:, 3:, :]], dim=1
            )
            anchor_embedding_tf = torch.cat(
                [anchor_embedding_tf, anchor_points[:, 3:, :]], dim=1
            )
        ...
        if self.flow_direction in ["both", "action2anchor"]:
            ...
            outputs["scores_action"] = flow_output_action["scores"][
                :, : action_points.shape[2], : anchor_points.shape[2]
            ]
        ...
        if self.flow_direction in ["both", "anchor2action"]:
            ...
            outputs["scores_anchor"] = flow_output_anchor["scores"][
                :, : anchor_points.shape[2], : action_points.shape[2]
            ]
...
class AlignedFrameDecoder(nn.Module):
    def __init__(
        ...
        self.emb_nn_action = DGCNN(
            emb_dims=self.emb_dims, input_dims=self.input_dims, conditioning_size=0
        )
        self.emb_nn_anchor = DGCNN(
            emb_dims=self.emb_dims, input_dims=self.input_dims, conditioning_size=0
        )
        ...
        self.transformer_action = Transformer(
            emb_dims=self.emb_dims, return_attn=True, bidirectional=False, n_heads=4
        )
        self.transformer_anchor = Transformer(
            emb_dims=self.emb_dims, return_attn=True, bidirectional=False, n_heads=4
        )
    ...
    def forward(
        ...
        *input,
        ...
        assert (
            action_center is not None and anchor_center is not None
        ), "Must provide action and anchor center"
        ...
        if self.flow_direction in ["both", "action2anchor"]:
            ...
            if self.head_output_type == "flow":
                ...
                if self.flow_frame == "aligned":
                    ...
                    pred_action_points_aligned_frame = (
                        action_points_dmean[:, :3, :]
                        + action_flow_and_weights[:, :3, :]
                    )
                    pred_action_points_anchor_frame = (
                        pred_action_points_aligned_frame + anchor_center
                    )
                    action_flow_anchor_frame = (
                        pred_action_points_anchor_frame - action_points[:, :3, :]
                    )
                    action_flow_and_weights = torch.cat(
                        [action_flow_anchor_frame, action_flow_and_weights[:, 3:, :]],
                        dim=1,
                    )
                ...
                if self.flow_frame == "aligned":
                    induced_flow_aligned_frame = (
                        action_points_and_weights[:, :3, :]
                        - action_points_dmean[:, :3, :]
                    )
                    action_flow_and_weights = torch.cat(
                        [
                    ...
                    pred_action_points_anchor_frame = (
                        action_points_and_weights[:, :3, :] + anchor_center
                    )
                    action_flow_anchor_frame = (
                        pred_action_points_anchor_frame - action_points[:, :3, :]
                    )
                    action_flow_and_weights = torch.cat(
                        [action_flow_anchor_frame, action_points_and_weights[:, 3:, :]],
                        dim=1,
                    )
        ...
        if self.flow_direction in ["both", "anchor2action"]:
            ...
            if self.head_output_type == "flow":
                ...
                if self.flow_frame == "aligned":
                    ...
                    # Transform the flow back to the original frame
                    pred_anchor_points_aligned_frame = (
                        anchor_points_dmean[:, :3, :]
                        + anchor_flow_and_weights[:, :3, :]
                    )
                    pred_anchor_points_action_frame = (
                        pred_anchor_points_aligned_frame + action_center
                    )
                    anchor_flow_action_frame = (
                        pred_anchor_points_action_frame - anchor_points[:, :3, :]
                    )
                    anchor_flow_and_weights = torch.cat(
                        [anchor_flow_action_frame, anchor_flow_and_weights[:, 3:, :]],
                        dim=1,
                    )
                ...
                if self.flow_frame == "aligned":
                    ...
                    pred_anchor_points_action_frame = (
                        anchor_points_and_weights[:, :3, :] + action_center
                    )
                    anchor_flow_action_frame = (
                        pred_anchor_points_action_frame - anchor_points[:, :3, :]
                    )
                    anchor_flow_and_weights = torch.cat(
                        [anchor_flow_action_frame, anchor_points_and_weights[:, 3:, :]],
                        dim=1,
                    )
