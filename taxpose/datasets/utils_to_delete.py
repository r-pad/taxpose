...
def __can_sample_init(dset_cls, dset_args):
    global __dataset
    __dataset = dset_cls(*dset_args)
    assert isinstance(dset_cls, CanSample)


def __paralel_sample(args: Tuple[int, Tuple, int, bool]):
    task_id = args[0]
    ...
    try:
        # Sample the data.
        data_list = []
        iterator = tqdm.tqdm(range(n_repeat)) if show_tqdm else range(n_repeat)
        for _ in iterator:
...
def parallel_sample(
    ...
    Args:
        ...
        dset_args (Tuple): The arguments to pass to the dataset constructor.
    ...
    ps_args = list(
        zip(
            range(len(get_data_args)),
            get_data_args,
            [n_repeat] * len(get_data_args),
            [n_proc == 0]
            * len(get_data_args),  # Only show tqdm internally if we're debugging.
        )
    )
    ...
    if n_proc == 0:
        ...
        __can_sample_init(dset_cls, dset_args)

        res = [__paralel_sample(args) for args in tqdm.tqdm(ps_args)]

    else:
        # Set the forking. Might fail.
        try:
        ...
        pool = multiprocessing.Pool(n_proc, __can_sample_init, (dset_cls, dset_args))
