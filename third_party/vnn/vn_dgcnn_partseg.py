# Taken from https://github.com/FlyingGiraffe/vnn/blob/master/models/vn_dgcnn_partseg.py
# Provides DGCNN implementation.
# Only changes:
...
class get_model(nn.Module):
    def __init__(self, args, num_part=50, normal_channel=False, gc=True, num_gc_classes=16):
        ...
        self.conv8 = nn.Sequential(nn.Conv1d(f_dim, 256, kernel_size=1, bias=False),
                               self.bn8,
                               nn.LeakyReLU(negative_slope=0.2))
        if self.gc:
            self.conv7 = nn.Sequential(nn.Conv1d(num_gc_classes, 64, kernel_size=1, bias=False),
                                    self.bn7,
                                    nn.LeakyReLU(negative_slope=0.2))
