...
class DGCNN_rgb(nn.Module):
    def __init__(self, emb_dims=512):
        super(DGCNN_rgb, self).__init__()
        # self.conv1 = nn.Conv2d(6, 64, kernel_size=1, bias=False)
        self.conv1 = nn.Conv2d(9, 64, kernel_size=1, bias=False)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=1, bias=False)
        self.conv3 = nn.Conv2d(64, 128, kernel_size=1, bias=False)
        self.conv4 = nn.Conv2d(128, 256, kernel_size=1, bias=False)
        self.conv5 = nn.Conv2d(512, emb_dims, kernel_size=1, bias=False)
    ...
    def forward(self, x, rgb):
        batch_size, num_dims, num_points = x.size()
        x = get_graph_feature(x)
        # CONCATENATING RGB FEATURES (NOTE: this is pretty hardcoded)
        rgb = rgb.unsqueeze(-1).expand(-1, -1, -1, x.shape[-1])
